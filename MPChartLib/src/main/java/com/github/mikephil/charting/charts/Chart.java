...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mData == null) {
            ...
            if (hasText) {
                // TODO: 2017/3/2 p00322 仿hellochart设置没有数据时图表显示 
                mInfoPaint.setStrokeWidth(Utils.convertDpToPixel(3));
                mInfoPaint.setColor(Color.parseColor("#DFDFDF"));
                int chartWidth = getWidth() - getPaddingLeft() * 4 * 2;
                int pointDistance = chartWidth / 3;
                Point point1 = new Point(getPaddingLeft() * 4, getHeight() - getPaddingBottom() * 8);
                Point point2 = new Point(point1.x + pointDistance, getPaddingTop() * 8);
                Point point3 = new Point(point2.x + pointDistance, (getHeight() - getPaddingBottom() * 8 + getPaddingTop() * 8) / 2);
                Point point4 = new Point(point3.x + pointDistance, getPaddingTop() * 8);
                float circleRadius = Utils.convertDpToPixel(6);
                canvas.drawCircle(point1.x, point1.y, circleRadius, mInfoPaint);
                canvas.drawLine(point1.x, point1.y, point2.x, point2.y, mInfoPaint);
                canvas.drawCircle(point2.x, point2.y, circleRadius, mInfoPaint);
                canvas.drawLine(point2.x, point2.y, point3.x, point3.y, mInfoPaint);
                canvas.drawCircle(point3.x, point3.y, circleRadius, mInfoPaint);
                canvas.drawLine(point3.x, point3.y, point4.x, point4.y, mInfoPaint);
                canvas.drawCircle(point4.x, point4.y, circleRadius, mInfoPaint);
                // TODO: 2017/3/2 p00322 注释没有数据的文字显示 
...
//                continue;
            ...
            if(mData.getDataSets().size()==2){
                tempEntry = new Entry[]{mData.getDataSets().get(0).getEntryForIndex((int)highlight.getX()),mData.getDataSets().get(1).getEntryForIndex((int)highlight.getX()),null};
                mMarker.refreshContent(tempEntry, highlight ,false);
            }else if(mData.getDataSets().size()==3){
                tempEntry = new Entry[]{mData.getDataSets().get(0).getEntryForIndex((int)highlight.getX()),mData.getDataSets().get(1).getEntryForIndex((int)highlight.getX()),mData.getDataSets().get(2).getEntryForIndex((int)highlight.getX())};
                mMarker.refreshContent(tempEntry, highlight ,true);
            }
            // draw the marker
