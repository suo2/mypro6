...
public class ChartHighlighter<T extends BarLineScatterCandleBubbleDataProvider> implements IHighlighter {
    ...
    protected float getMinimumDistance(List<Highlight> closestValues, float pos, YAxis.AxisDependency axis) {
        ...
        Log.d("closestValues:",closestValues.size()+"");

        for (int i = 0; i < closestValues.size(); i++) {

            Highlight high = closestValues.get(i);
            // TODO: 2017/3/8 p00322 修改组合报表点击柱状图和折线数值显示策略
            if (mChart instanceof CombinedChart&&closestValues.size()!=1) {
                if (high.getAxis() == axis) {
                    if (YAxis.AxisDependency.RIGHT != axis) {
                        ...
                        if (closestValues.get(0).getAxis() == YAxis.AxisDependency.RIGHT) {
                            leftYpx = closestValues.get(1).getYPx();
                            rightYpx = closestValues.get(0).getYPx();
                        } else {
                            leftYpx = closestValues.get(0).getYPx();
                            rightYpx = closestValues.get(1).getYPx();
                        }
                        ...
                        if (leftYpx >= rightYpx) {
                            ...
                            if (pos < leftYpx) {
                                ...
                                if (pos > rightYpx - 20 && pos < rightYpx + 20) {
                                    distance = Float.MIN_VALUE;
                                }
                            }
                        }
